...
static void check_computeblockversion(const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    BOOST_CHECK_EQUAL(g_versionbitscache.ComputeBlockVersion(nullptr, params, ALGO_SHA256D) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
    ...
    if (nTime == 0) {
        ...
        BOOST_CHECK_EQUAL(g_versionbitscache.ComputeBlockVersion(lastBlock, params, ALGO_SHA256D) & (1 << bit), 0);
        lastBlock = firstChain.Mine(params.nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((g_versionbitscache.ComputeBlockVersion(lastBlock, params, ALGO_SHA256D) & (1 << bit)) != 0);
    ...
    lastBlock = firstChain.Mine(params.nMinerConfirmationWindow * 3, nTime, ALGO_SHA256D).Tip();
    // so ComputeBlockVersion should now set the bit,
    BOOST_CHECK((g_versionbitscache.ComputeBlockVersion(lastBlock, params, ALGO_SHA256D) & (1 << bit)) != 0);
    // and should also be using the VERSIONBITS_TOP_BITS.
    BOOST_CHECK_EQUAL(g_versionbitscache.ComputeBlockVersion(lastBlock, params, ALGO_SHA256D) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
